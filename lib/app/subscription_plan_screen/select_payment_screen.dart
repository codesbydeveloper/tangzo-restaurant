...
import 'package:restaurant/controller/phonepay_controller.dart';
                ...
                onPress: () async {
                  if (controller.selectedPaymentMethod.value == '') {
                    ...
                    if (controller.selectedPaymentMethod.value == PaymentGateway.stripe.name.toLowerCase()) {
                      ...
                      await PhonePePaymentService.payNow(amountInPaise: (double.parse(controller.totalAmount.value.toString()) * 100).round());
                      ...
                      controller.makeInstamojoPayment(amount: controller.totalAmount.value.toString(), paymentDesc: "Subscription Payment");
                    } else if (controller.selectedPaymentMethod.value.toLowerCase() == controller.foloosiModel.value.name?.toLowerCase()) {
                      controller.makeFoloosiPayment(amount: controller.totalAmount.value.toString(), paymentDesc: "Subscription Payment");
                    } else if (controller.selectedPaymentMethod.value.toLowerCase() == controller.payMongoModel.value.name?.toLowerCase()) {
                      controller.makePayMongoPayment(amount: controller.totalAmount.value.toString(), paymentDesc: "Subscription Payment");
                    } else {
                      ShowToastDialog.showToast("Please select payment method");
                    }
                  }
