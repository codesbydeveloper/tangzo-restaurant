...
class OrderDetailsController extends GetxController {
  ...
  Future<void> calculatePrice() async {
    ...
    packagingCharge.value = double.parse(orderModel.value.vendor?.packagingCharge ?? '0.0');
  ...
  Future<dynamic> showLoadingAlert() {
    return showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: TranslatedText('Connect Bluetooth device'),
          content: SizedBox(
              ...
              child: availableBluetoothDevices.isEmpty
                  ? Column(
                      ...
                      children: [
                        Center(child: TranslatedText("Please connect device from your bluetooth setting.")),
                        Align(
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            title: TranslatedText(device.name),
                            subtitle: TranslatedText("Click to connect"),
                          );
                        },
                      ),
                      Align(
  ...
  Future<List<int>> getTicket() async {
    ...
    bytes += generator.row([
      ...
      PosColumn(
        text: orderModel.value.takeAway == false ? 'Deliver to door' : 'Takeaway',
        width: 5,
        styles: const PosStyles(
    ...
    generator.text(' ' * spaceBetweenColumns1, styles: const PosStyles());
    bytes += generator.row([
      PosColumn(text: 'Grand Total:', width: 5, styles: const PosStyles(align: PosAlign.left, height: PosTextSize.size1, width: PosTextSize.size1, bold: true)),
      PosColumn(
        text: '',
        width: 1, // Spacer column
      ),
    ...
    bytes += generator.row([
      PosColumn(
          text: '${'Bill By:'.tr} ${orderModel.value.vendor!.authorName}',
          width: 5,
          styles: const PosStyles(
            align: PosAlign.left,
            height: PosTextSize.size1,
    ...
    bytes += generator.hr(ch: '-', len: 32);
    generator.text(' ' * spaceBetweenColumns1, styles: const PosStyles());
    bytes += generator.text('Thank you very much', styles: const PosStyles(align: PosAlign.center, bold: true));
    bytes += generator.text('Please do come again!!', styles: const PosStyles(align: PosAlign.center, bold: true));
    generator.text(' ' * spaceBetweenColumns, styles: const PosStyles());
    bytes += generator.hr(ch: '-', len: 32);
    generator.text(' ' * spaceBetweenColumns, styles: const PosStyles());
    bytes += generator.cut();
